...
(print (acct-rcpt-table-template "transactions-example"))
#+end_src
...
In order to work with the template that has been defined in [[*Accounting Table Format][format]], we will need to define some functions to walk through the table and perform the needed work in verifying transactions.

*** TODO Study how to iterate over a named table

#+begin_src elisp :exports both :results raw :var data=transactions-example
(acct-rcpt-read-table data)
#+end_src
